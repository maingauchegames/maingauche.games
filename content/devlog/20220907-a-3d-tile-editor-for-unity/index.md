...
metavideo: /devlog/20220907-a-3d-tile-editor-for-unity/decorations.mov
draft: true
tags:
- stereoboy
- unity
...
You pick objects from a 3D-rendered selection palette, and create instances of them by clicking in the scene view.

{{< video-tag "./build.mov" >}}

Objects are automatically aligned to a 3D grid. The tool guarantees that you can't place overlapping objects in the same grid cell.
...
{{< video-tag "./elevation-on-select.mov" >}}
...
The way we *actually* added visual heterogeneity was by attaching decorations to pre-existing objects in the scene. This operation is common enough that we built it into Block Editor.

{{< video-tag "./decorations.mov" >}}
...
We thought it might be weird to have green trees to an ice level, so we made the decoration prefabs vary based on the skin of the parent object.

{{< video-tag "./decoration-skins.mov" >}}
...
There are a few circumstances where you want objects to overlap with each other. In Stereo Boy, the most common scenario is when a pressure switch sits under another object, such as a pushable block or patroller nav node.
